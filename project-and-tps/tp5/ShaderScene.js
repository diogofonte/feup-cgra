...
export class ShaderScene extends CGFscene {
	...
	init(application) {
		...
		this.testShaders[10].setUniformsValues({ uSamplerWaterTex: 2 });
		this.testShaders[10].setUniformsValues({ uSamplerWaterMap: 3 });
	...
	update(t) {
		...
		if (this.selectedExampleShader == 10)
			// Dividing the time by 100 "slows down" the variation (i.e. in 100 ms timeFactor increases 1 unit).
			// Doing the modulus (%) by 100 makes the timeFactor loop between 0 and 99
			// ( so the loop period of timeFactor is 100 times 100 ms = 10s ; the act
